...
const WhyFollowCard: React.FC<WhyFollowCardProps> = ({ icon, iconBg, title, desc }) => (
	<div className="flex flex-col items-center text-center px-2 min-w-[220px]">
		<div className={`rounded-2xl p-7 mb-6 flex items-center justify-center ${iconBg} shadow-sm`}>{icon}</div>
		<h3 className="font-bold text-2xl mb-2 text-gray-900 dark:text-gray-100">{title}</h3>
		<p className="text-gray-500 dark:text-gray-300 text-base max-w-xs">{desc}</p>
	</div>
);
...
const WhyFollowSection = () => (
	<section className="w-full flex justify-center mt-2 px-2">
		<div className="rounded-3xl bg-white dark:bg-card shadow-sm px-4 py-7 md:py-8 md:px-10 w-full max-w-7xl">
			...
			<div className="flex flex-col md:flex-row items-stretch justify-center gap-10 md:gap-16">
				...
				<WhyFollowCard
					...
					iconBg="bg-green-100 dark:bg-emerald-900/20"
					title="Real Projects"
