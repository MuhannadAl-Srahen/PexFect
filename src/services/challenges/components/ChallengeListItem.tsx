    ...
    >
      <Card className='overflow-hidden rounded-xl border border-border/50 bg-card shadow-sm hover:shadow-xl transition-transform duration-300 relative hover:scale-[1.02] py-0'>
        ...
        <div className='flex flex-col md:flex-row '>
          {/* Image Section */}
          <div className='relative w-full md:w-56 h-40 md:h-36 flex-shrink-0 overflow-hidden'>
            <img
            ...
            <div className='absolute top-3 left-3'>
              <Badge
                className={`${DIFFICULTY_COLORS[challenge.difficulty as DifficultyLevel]} px-3 py-1 text-xs font-semibold border-0 shadow-sm`}
              >
          ...
          <CardContent className='flex flex-1 flex-col !p-6'>
            <div className='flex items-start gap-3 justify-between'>
              <h3 className='mb-3 line-clamp-1 text-lg font-semibold text-card-foreground group-hover:text-primary transition-colors'>
                {challenge.title}
              </h3>
              <Button
                variant='ghost'
                size='sm'
                className='h-9 w-9 rounded-full bg-background/70 backdrop-blur-sm transition-all duration-300 flex-shrink-0'
                onClick={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
            ...
            {/* Tags + Stats */}
            <div className='mt-auto flex flex-wrap items-center justify-between gap-4 pt-3 border-t border-border/40'>
              {/* Tags */}
